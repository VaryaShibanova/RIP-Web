...
func (h *Handler) LoginUser(ctx *gin.Context) {
	...
	if err := h.SessionManager.CreateSession(ctx.Request.Context(), sessionID, session); err != nil {
		ctx.JSON(http.StatusInternalServerError, ErrorResponse{Error: "Ошибка создания сессии"})
		return
	}

	// Устанавливаем куки
	ctx.SetCookie("session_id", sessionID, h.Config.JWTExpiration*3600, "/", "", false, true)
